...
public class SaleDeliverDetailController {
    ...
    */
     ...
     * @date 2018-12-15
     */
    @PostMapping("/saleDeliverDetail/listPageDeliverDetail2Select")
    public ResultModel listPageDeliverDetail2Select()  throws Exception {
        logger.info("################saleDeliverDetail/listPageDeliverDetail2Select 执行开始 ################# ");
        Long startTime = System.currentTimeMillis();
        ResultModel model = new ResultModel();

        List<Column> columnList = columnService.findColumnList("deliverDetail2Select");
        if (columnList == null || columnList.size() == 0) {
            model.putCode("1");
            model.putMsg("数据库没有生成TabCol，请联系管理员！");
            return model;
        ...
        Pagination pg = HttpUtils.parsePagination(pd);
        List<Map> varMapList = new ArrayList();
        List<Map> varList = saleDeliverDetailService.getData2SelectListPage(pd,pg);
        if(varList!=null&&varList.size()>0){
            for(int i=0;i<varList.size();i++){
                Map map = varList.get(i);
                Map<String, String> varMap = new HashMap<String, String>();
        ...
        model.putResult(result);
        Long endTime = System.currentTimeMillis();
        logger.info("################saleDeliverDetail/listPageDeliverDetail2Select 执行结束 总耗时"+(endTime-startTime)+"ms ################# ");
        return model;
    }
