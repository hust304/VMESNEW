...
public class CustomerController {
    ...
    */
    ...
    public ResultModel deleteCustomers() throws Exception {
        ...
        Map<String, String> mapObject = customerService.findDeleteCustIdsByIds(ids);

        //禁用客户
        if (mapObject != null && mapObject.get("UpdateIds") != null && mapObject.get("UpdateIds").trim().length() > 0) {
            String updateIds = mapObject.get("UpdateIds").trim();
            customerService.updateToDisableByIds(updateIds.split(","));
        }

        //删除客户
        if (mapObject != null && mapObject.get("DeleteIds") != null && mapObject.get("DeleteIds").trim().length() > 0) {
            String deleteIds = mapObject.get("DeleteIds").trim();
            customerService.deleteByIds(deleteIds.split(","));
