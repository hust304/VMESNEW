...
public class TemplateController {
    ...
    */
     ...
     * @date 2018-08-23
     */
    @GetMapping("/test/listPageTestTemplates")
    public ResultModel listPageTestTemplates()  throws Exception {

        logger.info("################template/listPageTestTemplates 执行开始 ################# ");
        Long startTime = System.currentTimeMillis();
        HttpServletResponse response  = HttpUtils.currentResponse();
        ResultModel model = new ResultModel();
        PageData pd = HttpUtils.parsePageData();
        Pagination pg = HttpUtils.parsePagination(pd);
        Map result = new HashMap();
        List<Column> columnList = columnService.findColumnList("templateTest");
        ...
        List<Map> varList = templateService.getTestDataListPage(pd,pg);
        ...
        model.putResult(result);
        Long endTime = System.currentTimeMillis();
        logger.info("################template/listPageTestTemplates 执行结束 总耗时"+(endTime-startTime)+"ms ################# ");
        return model;
    }
     ...
     * @date 2018
     */
    @GetMapping("/test/listPageTest2Templates")
        public ResultModel listPageTest2Templates()  throws Exception {
        ...
        Pagination pg = HttpUtils.parsePagination(pd);
        Map result = new HashMap();
        List<Column> columnList = columnService.findColumnList("templateTest2");
        if (columnList == null || columnList.size() == 0) {
            model.putCode("1");
            model.putMsg("数据库没有生成TabCol，请联系管理员！");
            return model;
        ...
        List<Map> varMapList = new ArrayList();
        List<Map> varList = templateService.getTestDataList2Page(pd,pg);
        if(varList!=null&&varList.size()>0){
            for(int i=0;i<varList.size();i++){
                Map map = varList.get(i);
                Map<String, String> varMap = new HashMap<String, String>();
