...
//                    }
    ...
    public String checkColumnImportExcelBySpare(List<LinkedHashMap<String, String>> objectList,
                                         String companyId,
                                         Integer index,
                                         Integer maxShowRow) {
        StringBuffer strBuf = new StringBuffer();
        ...
        for (int i = 0; i < objectList.size(); i++) {
            ...
            //验证 (货品名称,规格型号,计量单位) 非空判断,(计量单位) 字典名称验证
            String msgStr = this.checkProductColumnBySpare(mapObject, (i+index_int));
            if (msgStr != null && msgStr.trim().length() > 0) {
                strBuf.append(msgStr);
                maxRow = maxRow + 1;
                if (maxShowRow_int <= maxRow) {return strBuf.toString();}
    ...
    public String checkColumnImportExcelByDataBaseBySpare(List<LinkedHashMap<String, String>> objectList,
                                                   String companyId,
                                                   Integer index,
                                                   Integer maxShowRow,
                                                   Map<String, String> productMap) {
        ...
        for (int i = 0; i < objectList.size(); i++) {
            ...
            //验证 (货品名称,规格型号,计量单位) 系统表中是否存在
            String msgStr = this.checkProductByDataBaseBySpare(mapObject, companyId, (i+index_int), productMap);
            if (msgStr != null && msgStr.trim().length() > 0) {
                strBuf.append(msgStr);
                maxRow = maxRow + 1;
                if (maxShowRow_int <= maxRow) {return strBuf.toString();}
    ...
    public void addWarehouseProductBySpare(Map<String, WarehouseProduct> warehouseProductMap, String companyId, String userId) throws Exception {
        if (warehouseProductMap == null || warehouseProductMap.size() == 0) {return;}
        ...
        for (Iterator iterator = warehouseProductMap.keySet().iterator(); iterator.hasNext();) {
            ...
            warehouseProduct.setCuser(userId);
            warehouseProduct.setCompanyId(companyId);
            warehouseProduct.setWarehouseId(warehouseSpare.getId());
    ...
    private String checkProductColumnBySpare(LinkedHashMap<String, String> mapObject, int index) {
        StringBuffer strBuf = new StringBuffer();
...
//            }
    ...
    private String checkProductByDataBaseBySpare(
            LinkedHashMap<String, String> mapObject,
            String companyId,
            int index,
            Map<String, String> productMap) {
        ...
        String msg_column_error = "第 {0} 行: {1}:{2} 输入错误，该{1}在系统中不存在！" + Common.SYS_ENDLINE_DEFAULT;
        String msg_column_error_1 = "第 {0} 行: (货品名称,规格型号,计量单位)输入错误，该货品在系统中不存在！" + Common.SYS_ENDLINE_DEFAULT;
        ...
        if (productCode != null && productCode.trim().length() > 0) {
            ...
            if (productMap.get(productCode) == null) {
                ...
                findMap.put("genre", "384cfd1329e648618b5a237ce1038bab");
                findMap.put("code", productCode);
                findMap.put("companyId", companyId);
                //是否启用(0:已禁用 1:启用)
                findMap.put("isdisable", "1");
